...
class WelfareResults:
    ...
    # Global (unconditional) summaries
    global_sweep: pd.DataFrame           # top-p rate for p = 1 … k
    rank_distribution: pd.DataFrame      # count / % at each rank + unmatched
    rank_stats: dict[str, float]         # avg rank, variance, std (matched only)
...
def build_student_level_welfare(
    ...
    Parameters
    ----------
    priority_higher_is_better : bool, default True
        Set to False for raw lottery numbers where lower value = better priority.
        Controls the ascending flag in percentile computation — see _resolve_priority.
    """
...
# Only matched students contribute (NaN = unmatched, excluded).
# ---------------------------------------------------------------------------
...
def _grouped_sweep(
    ...
    """Shared loop: top-p rate + rank stats for every (p, group) pair."""
    frames = []
...
def summarize_top_p_sweep_by_priority_percentile(
    ...
    # avg_priority_percentile added separately since _grouped_sweep is generic
...
def evaluate_simulation_output(
    ...
    ----------
    sim_output : dict
        Required keys: "rankings_as_indices", "matches_idx", "student_attributes".
    priority_higher_is_better : bool, default True
        True  → higher priority_score = better student (e.g. test score).
        False → lower score = better student (raw lottery numbers).
        Directly sets ascending in pandas rank() — see _resolve_priority.
    """
